# Terms of Service

*Last updated: August 13, 2026 — draft; review before store submission.*

These Terms of Service ("Terms") govern your use of the Jackdaw browser extension and related services ("the Service"). By using the Service you agree to these Terms.

## 1. The Service

Jackdaw is a free, open-source tool that displays community-contributed price history and discussion for Micro Center products. Jackdaw is not affiliated with, endorsed by, or sponsored by Micro Center or Micro Electronics, Inc. All product names and trademarks are the property of their respective owners.

## 2. Community Data

Price data is contributed by users' own browsers from pages they visit. It is provided "as is," may be incomplete or inaccurate, and is not an offer or guarantee of any price. Confirm prices with the retailer before purchasing.

## 3. User Content

You may post comments, votes, and reports. You retain ownership of content you post and grant us a non-exclusive, royalty-free license to display and distribute it within the Service. You may not post content that is unlawful, harassing, or defamatory; spam or commercial solicitation; links or contact information; or content you do not have the right to share. We may hide or remove content and restrict access for violations, with or without notice. Content hidden by community reports may be reviewed by a maintainer.

## 4. Disclaimer of Warranty; Limitation of Liability

The Service is provided "as is" and "as available," without warranty of any kind, express or implied. To the maximum extent permitted by law, the maintainers are not liable for any indirect, incidental, or consequential damages arising from use of the Service or reliance on its data.

## 5. Copyright Complaints

If you believe content on the Service infringes your copyright, submit a notice containing the information required by 17 U.S.C. § 512(c)(3) via an issue in the project's GitHub repository. We will respond promptly.

## 6. Changes

We may update these Terms. Continued use after an update constitutes acceptance. The current version is maintained in the project repository.

## 7. Governing Law

These Terms are governed by the laws of the State of Ohio, USA, without regard to conflict-of-law principles.
