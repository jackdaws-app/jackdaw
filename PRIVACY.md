...
- No name, email address, postal address, or account credentials.
- No browsing history. The extension runs only on `microcenter.com` product pages and collects nothing elsewhere.
...
Data is stored with our backend provider, Convex (United States). Aggregate price history is public by design: it is displayed to all users and may be published in open formats. Comments and display names are visible to all users. Device identifiers and alert targets are not publicly displayed.

## 5. Data Retention and Deletion

Uninstalling the extension deletes the device identifier from your browser. Previously contributed price observations and comments remain in the community database; they are not linked to any identifying information. To request removal of specific content you posted, open an issue in the project's GitHub repository.
