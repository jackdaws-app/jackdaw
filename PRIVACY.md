...
Some information is held only in your browser and is never transmitted to us: your extension settings, and a short list of the readings the extension has recently reported, which it keeps so that revisiting or re-filtering the same listings does not report the same item repeatedly. Each entry records the reading it is there to recognise, which is the same information already described in Section 1 as reported to us: the product and store identifiers, and the price, stock and availability details read from the listing. Entries stop counting after a few minutes, are discarded the next time the extension reads a listing page, and are removed when you uninstall the extension.
